...
//
	...
	if (!linked)
	{
        ...
        if ((file = fopen("/Users/unicorn1343/metalShaders/3.txt","a")) == NULL)
            printf("Файл невозможно открыть или создать\n");
        else
            fwrite (vp_ptr, sizeof(char),vp_size, file );
        
            fclose(file);
        
        if ((file = fopen("/Users/unicorn1343/metalShaders/2.txt","a")) == NULL)
            printf("Файл невозможно открыть или создать\n");
        else
            fwrite (fp_ptr, sizeof(char),fp_size, file );
